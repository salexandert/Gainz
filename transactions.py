...
def _restore_transaction_economics(transaction, row):
    transaction.set_economics(
        ...
        source_quantity_text=_first_row_value(row, "source_quantity_text") or "",
        source_usd_total=_first_row_value(row, "source_usd_total"),
        implied_usd_total=_first_row_value(row, "implied_usd_total"),
        value_variance_usd=_first_row_value(row, "value_variance_usd"),
        value_tolerance_usd=_first_row_value(row, "value_tolerance_usd"),
        input_reliability_status=(
            _first_row_value(row, "input_reliability_status") or "NOT_CHECKED"
        ),
        source_leg=_first_row_value(row, "source_leg") or "",
    )
    return transaction
...
class Transactions:
    ...
    def load(self, filename=None):
        ...
        self.import_receipts = self._load_records_sheet(
            workbook,
            "Import Row Receipts",
            IMPORT_RECEIPT_COLUMNS,
        )
    ...
    def _load_tax_year_records(self, workbook):
        ...
        sheet = workbook['Tax Year Records']
        headers = [
            str(cell.value).strip()
            for cell in sheet[1]
            if cell.value is not None
        ...
        for values in sheet.iter_rows(min_row=2, values_only=True):
            ...
            records.append({
                "year": year,
                "reported_proceeds": _float_or_none(row.get("reported_proceeds")),
                "reported_cost_basis": _float_or_none(row.get("reported_cost_basis")),
                "reported_gain_loss": _float_or_none(row.get("reported_gain_loss")),
                "tax_paid": _float_or_none(row.get("tax_paid")),
                "filing_status": _string_or_empty(row.get("filing_status")) or "Filed",
                "evidence_reference": _string_or_empty(row.get("evidence_reference")),
                "notes": _string_or_empty(row.get("notes")),
                "updated_at": _string_or_empty(row.get("updated_at")),
            })
    ...
    def get_tax_year_record(self, year):
        year = int(year)
        for record in getattr(self, "tax_year_records", []):
            if int(record.get("year")) == year:
                return record

        return None

    def set_tax_year_record(
        self,
        year,
        ...
        record = {
            "year": int(year),
            "reported_proceeds": _float_or_none(reported_proceeds),
            "reported_cost_basis": _float_or_none(reported_cost_basis),
            "reported_gain_loss": _float_or_none(reported_gain_loss),
            "tax_paid": _float_or_none(tax_paid),
            "filing_status": filing_status or "Filed",
        ...
        records = [
            existing
            for existing in getattr(self, "tax_year_records", [])
            if int(existing.get("year")) != record["year"]
        ]
        records.append(record)
    ...
    def save(self, description=None):
        ...
        import_receipts_df = pd.DataFrame(
            getattr(self, "import_receipts", []),
            columns=IMPORT_RECEIPT_COLUMNS,
        )
        ...
        with pd.ExcelWriter(save_as_filename, engine='xlsxwriter') as writer:
            ...
            import_receipts_df.to_excel(writer, sheet_name="Import Row Receipts", index=False)
    ...
    def _write_export_status_sheet(self, workbook, readiness=None):
        ...
        rows = [
            ...
            ("Generated by", "Gainz local workbook export"),
            ("Important", "Documentation support only. Review source records before filing."),
        ]
        input_reliability = readiness.get("input_reliability") or {}
        if not input_reliability.get("passed", True):
            rows.append((
                "INPUT RELIABILITY",
                "FAILED - Sales and Form 8949 calculation sheets are suppressed until source import errors are corrected.",
            ))
        ...
        write_row = 10
        sheet.cell(row=write_row, column=1).value = "Open blockers"
        sheet.cell(row=write_row, column=1).font = Font(bold=True)
        write_row += 1
        blockers = readiness.get("blockers") or []
        if blockers:
            for blocker in blockers:
                sheet.cell(row=write_row, column=1).value = blocker
                write_row += 1
        else:
        ...
        write_row += 1
        sheet.cell(row=write_row, column=1).value = "Open warnings"
        sheet.cell(row=write_row, column=1).font = Font(bold=True)
        write_row += 1
        warnings = readiness.get("warnings") or []
        if warnings:
            for warning in warnings:
                sheet.cell(row=write_row, column=1).value = warning
                write_row += 1
        else:
            sheet.cell(row=write_row, column=1).value = "None"
    ...
    def _write_import_economics_sheet(self, workbook):
        ...
        columns = [
            ...
            ("interpreted_quantity", "Interpreted Quantity"),
            ("usd_spot", "USD Spot"),
            ("gross_usd", "Gross USD"),
            ("fee_usd", "Fee USD"),
            ...
            ("implied_usd", "Implied USD"),
            ("value_variance_usd", "Variance USD"),
            ("value_tolerance_usd", "Tolerance USD"),
            ("input_reliability_status", "Input Reliability"),
    ...
    def export_to_excel(self, asset=None, date_range=None, by_year=True, output_dir=None, readiness=None):
        ...
        input_reliability_failed = not (readiness or {}).get(
            "input_reliability",
            {"passed": True},
        ).get("passed", True)
        ...
        form_rows = [] if input_reliability_failed else get_form_8949_report_rows(self)
