...
Windows may show a SmartScreen warning and macOS may show an operating-system trust prompt. Only download Gainz from `cryptogainz.store` or the official GitHub repository, then verify the published checksum. See [Windows SmartScreen Warning For Gainz]({{ '/guides/windows-smartscreen/' | relative_url }}) before continuing through a Windows warning.
