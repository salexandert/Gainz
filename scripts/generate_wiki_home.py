...
SCREENSHOTS = [
    ...
    (
        ...
        "Confirm how Gainz interpreted quantity, source gross value, fees, and total cost or net proceeds before continuing.",
    ),
    (
        "Reconcile",
        "gainz-holdings-accounting.png",
        "Work one holdings or missing-basis gap at a time with numbered steps and a plain-language explanation.",
    ),
    (
        "Professional treatment preview",
        "gainz-professional-resolution-preview.png",
        "Review source gross value, allocated fee, net proceeds, and exact before/after Form 8949 impact before applying a reversible treatment.",
    ),
    (
...
def build_home():
    ...
    lines.extend(
        [
            ...
            f"- [Importer Coverage And Economic Fields]({_repo_doc_url(DOCS_ROOT / 'importer-coverage.md')})",
...
def build_docs_flow():
    ...
    lines = [
        ...
        "## Source Of Truth",
        "",
        "Update `docs/` first for product documentation. This wiki is generated from the same Markdown and synthetic screenshots. The public website remains in the separate `Gainz-Website` repository and syncs versioned public assets from the app repository.",
        "",
        ...
        "- `docs/importer-coverage.md` for native/mapped importer economics and limitations.",
        ...
        "## Automation",
        "",
        f"- Netlify publishes the separate `Gainz-Website` repository at <{site_url}>.",
        "- `.github/workflows/sync-website.yml` dispatches the separate website sync after version, docs, screenshot, or sample-output changes.",
        "- The website sync copies synthetic screenshots and sample downloads, validates sample version/checksum metadata, and updates release text.",
